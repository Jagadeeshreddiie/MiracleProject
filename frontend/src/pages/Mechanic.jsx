...
const Mechanic = () => {
    ...
    useEffect(() => {
        const fetchMechanics = async () => {
            try {
                const response = await fetch("https://miracle-project-backend1.vercel.app/mechanics/");
                if (!response.ok) {
  ...
  if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
          position => {
              ...
              navigate("/book", { state: { latitude: newLatitude, longitude: newLongitude } }); // Navigate with latitude and longitude as props
          },
          error => {
    ...
    return (
        <div className="main-home diagnosis">
            ...
            <div className="diag-home mechanic-home" style={{ marginRight: '10px', gap: '10px' }}>
                {mechanics.map((each, index) => {
                    return (
                        <div key={index} className="card" style={{ marginRight: '10px' }}>
                            <p>Name : {each.name}</p>
                            <p>Special in : {each.special}</p>
                            <p>Experience of <span style={{ fontSize: '26px', color: 'red' }}>{each.experience} </span>Years</p>
